...
import Rightarrow from "@/components/assets/icons/Rightarrow";
...
const NextArrow = ({ onClick }: any) => (
    <div
        onClick={onClick}
        className="absolute top-[42%] -right-10 lg:-right-7"
    >
        <Rightarrow />
    </div>
);
...
export default class SliderForResultCards extends Component<{ offer: boolean, carouselData: Array<internshipArrayCrouselType> }>{
            ...
            >
                <Slider {...this.settings}>
                    ...
                    {this.props.carouselData.map((item: internshipArrayCrouselType, index: number) => (
                        <main key={index}>
                            <CarouselCard data={item} offer={this.props.offer}/>
                        </main>
                    ))}
                </Slider>
            </div>
